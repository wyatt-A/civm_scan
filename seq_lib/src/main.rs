...
use glob::glob;
use seq_lib::fse_dti::{FseDti, FseDtiParams};
use seq_lib::pulse_sequence::{Build,Setup,Protocol};
use regex::Regex;
...
fn main(){
    ...
    match &args.action {
        ...
        New(args) => new(args),
        NewDiffusionExperiment(args) => new_diffusion_experiemnt(args),
...
fn new_config(args:&NewConfigArgs){
...
fn new_diffusion_experiemnt(args:&NewDiffusionExperimentArgs){
    let cfg_file = Path::new(SEQUENCE_LIB).join(&args.alias).with_extension("json");
...
fn apply_setup(args:&ApplySetupArgs) {
